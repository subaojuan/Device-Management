...
<c:if test="${empty requestScope.pageBean.list}">
  ...
  <td colspan="6">目前没有设备可以用</td>
     <td><a href="">修改设备</a></td>
  <td><a href="">删除设备</a></td>
  <td><a href="./backend/addComputer.jsp">增加设备</a></td>
 </tr> 
 </c:if>
...
<c:forEach items="${requestScope.pageBean.list}" var="a">
 <tr>
  ...
  <td>${a.laboratory}</td>
  <td>${a.isUse}</td>
  <td><a href="/DeviceMangement/queryComputerDetailServlet?number=${a.number}">修改设备</a></td>
  <td><a href="/DeviceMangement/deleteCompterServlet?number=${a.number}">删除设备</a></td>
  <td><a href="./backend/addComputer.jsp">增加设备</a></td>
  </tr>
</c:forEach>
...
<%-- 构建分页导航 --%>
            ...
            <br/>
            <a href="${pageContext.request.contextPath}/QueryServletComputer?pageNumber=1">首页</a>
            ...
            <c:if test="${requestScope.pageBean.pageNumber==1 && requestScope.pageBean.totalPage!=1}">
                <c:forEach begin="${requestScope.pageBean.start}" end="${requestScope.pageBean.end}" step="1" var="i">
                    ...
                    <c:if test="${requestScope.pageBean.pageNumber != i}">
                        <a href="${pageContext.request.contextPath}/QueryServletComputer?pageNumber=${i}">${i}</a>                                        
                    </c:if>                        
                </c:forEach>
                <a href="${pageContext.request.contextPath}/QueryServletComputer?pageNumber=${requestScope.pageBean.pageNumber+1}">下一页</a>                    
            </c:if>
            
            <%--如果当前页不是第一页也不是最后一页，则有上一页和下一页这个超链接显示 --%>
            <c:if test="${requestScope.pageBean.pageNumber > 1 && requestScope.pageBean.pageNumber < requestScope.pageBean.totalPage}">
                <a href="${pageContext.request.contextPath}/QueryServletComputer?pageNumber=${requestScope.pageBean.pageNumber-1}">上一页</a>
                <c:forEach begin="${requestScope.pageBean.start}" end="${requestScope.pageBean.end}" step="1" var="i">    
                    <c:if test="${requestScope.pageBean.pageNumber == i}">
                        ${i}
                    </c:if>            
            ...
            <%--尾页 --%>
            <a href="${pageContext.request.contextPath}/QueryServletComputer?pageNumber=${requestScope.pageBean.totalPage}">尾页</a>
</div>
